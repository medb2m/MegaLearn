...
export class AddEditCourseComponent implements OnInit {
  ...
  onSubmit() {
    ...
    formData.append(
      'sections',
      JSON.stringify(this.form.get('sections')?.value)
    );
    ...
    if (this.id) {
      ...
      if (this.idCat !== this.form.value.category._id) {
        this.catService
          .associate(this.form.value.category, this.courseId)
          .pipe(first())
          .subscribe((x) => {
            console.log(x);
          });
        ...
        this.catService
          .disassociate(this.idCat, this.courseId)
          .pipe(first())
          .subscribe((x) => {
            console.log(x);
          });
    ...
    saveCourse()
      ...
      .subscribe({
        next: (c) => {
          console.log(c);
          this.catService
            .associate(this.form.value.category, c._id)
            .pipe(first())
            .subscribe((x) => {
              console.log(x);
            });
